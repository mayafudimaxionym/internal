    ...
    if files_to_process:
        for file_path, file_hash in tqdm(files_to_process, desc="Converting text to speech"):
            try:
                ...
                if not text_content.strip(): logger.warning(f"Skipping empty file: {file_path}"); continue
                ...
                if isinstance(audio_data, bytes):
                    ...
                    logger.error(f"Failed to convert {file_path} with {current_key_name}: {audio_data}"); failed_count += 1
            except Exception as e:
                logger.error(f"An unexpected error with {current_key_name} on {file_path}: {e}"); failed_count += 1
...
def run_slides(args, logger):
    """Finds .gslides files and exports them by matching normalized names."""
    ...
    slides_map = get_all_slides_map(drive_service, logger)
    if slides_map is None: return False
    normalized_slides_map = {normalize_name(name): data for name, data in slides_map.items()}
    state = load_state(args.module_path)
    ...
    if hasattr(args, 'video_folder') and args.video_folder:
        ...
        if not os.path.isdir(specific_folder_path):
            logger.error(f"Specified video folder not found: {specific_folder_path}"); return False
        search_path = specific_folder_path
        logger.info(f"Processing in targeted mode: scanning folder '{args.video_folder}'.")
    else:
    ...
    for root, _, files in os.walk(search_path):
        for file in files:
            if file.endswith(".gslides"):
                ...
                normalized_local_name = normalize_name(base_name)
                match = normalized_slides_map.get(normalized_local_name)
                if not match:
                    logger.warning(f"Could not find a match on Google Drive for '{base_name}'. Skipping.")
                    continue
                presentation_id, remote_mod_time = match['id'], match['modifiedTime']
    ...
    logger.info(f"Found {len(presentations_to_process) + skipped_count} matching presentations. {len(presentations_to_process)} to process, {skipped_count} up to date.")
        ...
        try:
            ...
            for i, page in enumerate(pages):
                ...
                if response.status_code == 200:
                    ...
                    logger.error(f"Failed to download thumbnail for slide {slide_num}. Status: {response.status_code}"); failed_slides += 1
            relative_path = os.path.relpath(gslides_path, os.path.abspath(args.module_path))
            state["files"][relative_path] = mod_time
        except Exception as e:
            logger.error(f"Failed to process presentation {pres_id}: {e}"); failed_slides += len(pages) if 'pages' in locals() else 1
    save_state(args.module_path, state)
    end_time = time.time()
...
def run_tts(args, logger):
    """Converts .txt files to .wav, saving audio files in the parent 'Video' folder."""
    logger.info("--- Starting: Text-to-Speech Conversion ---")
    start_time = time.time()
    api_keys = load_api_keys(logger)
    if not api_keys: return False
    tts_config = load_tts_config(logger)
    if not tts_config: return False
    model_name, prompt_template, delay = tts_config.get("model_name"), tts_config.get("prompt_template", "{}"), tts_config.get("delay_seconds", 5)
    state = load_state(args.module_path)
    key_stats = state.get("key_stats", {})
    for i in range(len(api_keys)):
        if f"key_{i}" not in key_stats: key_stats[f"key_{i}"] = 0
    ...
    for root, _, files in os.walk(search_path):
        for file in files:
            if file.endswith(".txt") and "course_outline" not in file:
                total_files += 1; file_path = os.path.join(root, file)
                current_hash = get_file_hash(file_path)
                relative_path = os.path.relpath(file_path, args.module_path)
                if state["files"].get(relative_path) == current_hash: skipped_files += 1
                else: files_to_process.append((file_path, current_hash))
    logger.info(f"Found {total_files} total text files. {len(files_to_process)} to process, {skipped_files} already up to date.")
    converted_count, failed_count, key_index = 0, 0, 0
    if files_to_process:
        for file_path, file_hash in tqdm(files_to_process, desc="Converting text to speech"):
            try:
                ...
                if isinstance(audio_data, bytes):
                    ...
                    slide_folder_path = os.path.dirname(file_path)
                    video_folder_path = os.path.dirname(slide_folder_path)
                    wav_filename = os.path.splitext(os.path.basename(file_path))[0] + ".wav"
                    output_path = os.path.join(video_folder_path, wav_filename)
                    # --- MODIFIED BLOCK END ---

                    with wave.open(output_path, "wb") as wf:
                        wf.setnchannels(1); wf.setsampwidth(2); wf.setframerate(24000); wf.writeframes(audio_data)
                    relative_path = os.path.relpath(file_path, args.module_path)
                    state["files"][relative_path] = file_hash; converted_count += 1
                    key_stats[current_key_name] += 1
                    logger.info(f"Success with {current_key_name} for {os.path.basename(file_path)}. Pausing for {delay} sec...")
                    key_index = (key_index + 1) % len(api_keys)
                    time.sleep(delay)
...
def run_slides(args, logger):
    """Exports Google Slides to PNGs, saving images in the parent 'Video' folder."""
    logger.info("--- Starting: Google Slides Export ---")
    start_time = time.time()
    drive_service, slides_service = get_google_api_services(logger, args.credentials)
    if not drive_service or not slides_service: return False
    ...
    for root, _, files in os.walk(search_path):
        for file in files:
            if file.endswith(".gslides"):
                ...
                presentation_id, remote_mod_time = match['id'], match['modifiedTime']
                relative_path = os.path.relpath(gslides_path, os.path.abspath(args.module_path))
                if state["files"].get(relative_path) == remote_mod_time: skipped_count += 1
                else: presentations_to_process.append((gslides_path, presentation_id, remote_mod_time))
    logger.info(f"Found {len(presentations_to_process) + skipped_count} matching presentations. {len(presentations_to_process)} to process, {skipped_count} up to date.")
    exported_slides, failed_slides = 0, 0
    for gslides_path, pres_id, mod_time in tqdm(presentations_to_process, desc="Exporting slides"):
        try:
            ...
            logger.info(f"Processing '{os.path.basename(gslides_path)}' ({len(pages)} slides)...")
            for i, page in enumerate(pages):
                slide_num, page_id = i + 1, page['objectId']
                video_folder = os.path.dirname(gslides_path)
                slide_folder = os.path.join(video_folder, f"Slide {slide_num}")
                if not os.path.exists(slide_folder): os.makedirs(slide_folder)
                thumbnail_url = slides_service.presentations().pages().getThumbnail(
                ...
                if response.status_code == 200:
                    # --- MODIFIED LINE ---
                    output_path = os.path.join(video_folder, f"Slide {slide_num}.png")
                    with open(output_path, 'wb') as f: f.write(response.content)
                    exported_slides += 1
                else:
                    logger.error(f"Failed to download thumbnail for slide {slide_num}. Status: {response.status_code}"); failed_slides += 1
