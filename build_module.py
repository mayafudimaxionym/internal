...
def save_state(module_path, state):
    """Saves the build state to .build_status.json."""
    state_file = os.path.join(module_path, ".build_status.json")
...
def parse_outline(file_path, logger):
    """Parses a Markdown-formatted course outline file."""
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            lines = f.readlines()
    except FileNotFoundError:
        logger.error(f"Outline file not found at {file_path}")
        return None
    except Exception as e:
        logger.error(f"An error occurred reading the outline file: {e}")
        return None
    ...
    for line in lines:
        ...
        if line.startswith('# '):
            structure["module_title"] = line[2:].strip()
...
def load_tts_config(logger):
    """Loads TTS configuration from tts_config.json."""
...
def get_google_api_services(logger, credentials_path):
    ...
    if not os.path.exists(credentials_path):
        logger.error(f"FATAL: Service account key file not found at '{credentials_path}'")
        return None, None
    ...
    try:
        ...
        logger.error(f"Failed to authenticate using service account: {e}")
        return None, None
...
def run_structure(args, logger):
    ...
    if not course_data:
        logger.error("Failed to parse course outline. Aborting structure generation.")
        return
    ...
    for video_idx, video in enumerate(course_data["videos"], 1):
        video_folder_name = f"Video {video_idx}"
        video_path = os.path.join(args.module_path, video_folder_name)
        if not os.path.exists(video_path):
            os.makedirs(video_path)
            video_dirs_created += 1

        for slide_idx, slide in enumerate(video["slides"], 1):
            slide_folder_name = f"Slide {slide_idx}"
            slide_path = os.path.join(video_path, slide_folder_name)
            if not os.path.exists(slide_path):
                os.makedirs(slide_path)
                slide_dirs_created += 1
...
def run_tts(args, logger):
    """Finds all .txt files and converts them to .wav audio."""
    ...
    for root, _, files in os.walk(args.module_path):
        for file in files:
    ...
    converted_count, failed_count = 0, 0
    if files_to_process:
        for file_path, file_hash in tqdm(files_to_process, desc="Converting text to speech"):
            try:
                ...
                if isinstance(audio_data, bytes):
                    ...
                    converted_count += 1
                    logger.info(f"Success for {os.path.basename(file_path)}. Pausing for {delay} seconds...")
                    time.sleep(delay)
                else:
                    logger.error(f"Failed to convert {file_path}: {audio_data}")
                    failed_count += 1
            except Exception as e:
                logger.error(f"An unexpected error occurred processing {file_path}: {e}")
                failed_count += 1
...
def run_slides(args, logger):
    ...
    state = load_state(args.module_path)
    module_path_abs = os.path.abspath(args.module_path)
    logger.info(f"Scanning for presentations in absolute path: {module_path_abs}")

    presentations_to_process, skipped_count = [], 0
    
    for root, _, files in os.walk(module_path_abs):
        for file in files:
            if file.endswith(".gslides"):
                ...
                try:
                    ...
                    if not response['files']:
                        logger.warning(f"Could not find presentation named '{base_name}' on Google Drive. Skipping.")
                        continue
                    
                    drive_file = response['files'][0]
                    presentation_id, remote_mod_time = drive_file['id'], drive_file['modifiedTime']
                    
                    relative_path = os.path.relpath(gslides_path, module_path_abs)
...
def run_build(args, logger):
    ...
    # The slides step is optional and depends on the --credentials argument
...
def main():
    ...
    sp_tts = subparsers.add_parser("tts", help="Generate audio files from text.", parents=[parent_parser])
    sp_tts.set_defaults(func=run_tts)

    sp_slides = subparsers.add_parser("slides", help="Export Google Slides to PNG.", parents=[parent_parser])
    ...
    sp_build = subparsers.add_parser("build", help="Run the full build process (structure, tts, slides).", parents=[parent_parser])
    ...
    if not os.path.isdir(args.module_path):
        print(f"Error: Module directory not found at '{args.module_path}'")
        return
